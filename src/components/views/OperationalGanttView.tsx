...
export const OperationalGanttView: React.FC = () => {
  ...
  const { data: ganttData = [], isLoading, refetch } = useQuery<OperationalTask[]>({
    ...
    queryFn: async () => {
      // Data representation for live demonstration
      return [
        { id: 't1', duty_number: 1, vehicle_id: '4001', driver_name: 'Петренко О.М.', type: 'PULL_OUT', label: '🚩 Виїзд з депо', start_min: 30, duration_min: 25, color: '#4f46e5' },
        ...
        { id: 't5', duty_number: 2, vehicle_id: '4002', driver_name: 'Ковальчук В.І.', type: 'PULL_OUT', label: '🚩 Виїзд з депо', start_min: 36, duration_min: 25, color: '#4f46e5' },
        ...
        { id: 't8', duty_number: 3, vehicle_id: '4003', driver_name: 'Сидоренко Г.П.', type: 'PULL_OUT', label: '🚩 Виїзд з депо', start_min: 42, duration_min: 25, color: '#4f46e5' },
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 dark:border-slate-800 pb-4">
          <div className="space-y-1">
            <div className="flex items-center space-x-2">
              <span className="px-2.5 py-0.5 rounded-md bg-purple-50 dark:bg-purple-950 text-purple-700 dark:text-purple-300 border border-purple-200 dark:border-purple-800 font-extrabold text-[11px] uppercase tracking-wider">
                LIVE GANTT CAD/AVL
              </span>
          ...
          <div className="flex items-center space-x-3 shrink-0">
            ...
            <button
              onClick={() => refetch()}
              className="bg-blue-600 hover:bg-blue-700 text-white font-extrabold text-xs px-4 py-2 rounded-xl shadow-md shadow-blue-600/20 flex items-center space-x-1.5 cursor-pointer transition-all active:scale-95"
            >
        ...
        <div className="overflow-x-auto pt-2 pb-4">
          <div className="min-w-[1000px] space-y-3 font-mono text-xs">
            ...
            <div className="flex border-b border-slate-200 dark:border-slate-700 pb-2 text-[10px] text-slate-400 font-bold">
              <div className="w-36 shrink-0 font-sans uppercase text-slate-600 dark:text-slate-400 font-extrabold">Наряд / Вагон</div>
              <div className="flex-1 flex justify-between relative pl-2">
                {hoursMarks.map(h => (
                  <span key={h} className="text-center w-8 text-slate-600 dark:text-slate-400 font-extrabold">
                    {h < 10 ? `0${h}:00` : `${h}:00`}
                  </span>
                ))}
              </div>
            ...
            {duties.map(dNum => {
              ...
              return (
                <div key={dNum} className="flex items-center hover:bg-blue-50/50 dark:hover:bg-slate-800/40 p-1.5 rounded-xl transition-colors">
                  <div className="w-36 shrink-0 font-sans text-xs font-black text-slate-800 dark:text-slate-200">
                    <div>Наряд №{dNum}</div>
                    <div className="text-[10px] text-blue-700 dark:text-blue-400 font-mono font-bold">
                      Вг-{sample.vehicle_id}
                    </div>
                  </div>

                  <div className="flex-1 h-8 bg-slate-100 dark:bg-slate-800 rounded-xl relative overflow-hidden border border-slate-200/60 dark:border-slate-700">
                    {dTasks.map(task => {
